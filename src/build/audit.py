...
def valid_ref(cite: dict) -> bool:
    """引用要嘛有合法 PMID（生醫），要嘛有合法 DOI（人文社科）——只查格式，真偽交給 verify_refs.py。"""
    pmid = str(cite.get("pmid") or "").strip()
    if pmid.isdigit() and 6 <= len(pmid) <= 9:
        return True
    doi = str(cite.get("doi") or "").strip()
...
def audit_depth(cfg: dict, units: list[dict], opts: dict, rep: Report) -> None:
    ...
    if bad_ref:
        rep.err(
            sec,
            f"{len(bad_ref)} 筆引用既沒有合法 PMID 也沒有合法 DOI"
            "（跑 make verify 打真實 API 才算數）",
            bad_ref,
        )
    if thin_cats:
